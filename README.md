...
- [ ] Encourage, but don't require pull-request workflow
...
- [ ] Have `env-shell.sh` print combo's git commit id. Also log the current datetime and the commit id to logfile on icetray execution.
...
- [ ] A series of screencasts helps the adoption of git by helping people and by showing them the advantages — I do have the neccessary equipment and volunteer

## Cons

- There is a time delay of possibly several seconds after pushing to master, after which a push can be rejected and reset.
- People that love the beauty of linear commit histories may be disappointed at first (but we can still provide a beatified `git log` command)
...
- People can still push to master or to trunk on svn; but we do encourage pull requests and code reviews; it has a nice feel to get code reviewed and approved

## Other implications

## Tests, experiments, and checks

- [x] Tried to merge a project (monopole-generator) into combo preserving both histories
- [x] Using svn externals for meta projects
- [x] Using gitman for meta projects
- [ ] Importing pre-combo history while preserving the existing combo git history
- [x] Use github actions to run build and tests

## Wishlists, questions, and discussions

### Nathan Whitehorn (https://github.com/fiedl/icecube-git-migration/issues/5)
...
- [x] Git migration of combo drops svn commits before Oct 2018
  - We can still include other histories (which I am in favour for)
...
### Erik Blaufuss, Don La Dieu (https://github.com/fiedl/icecube-git-migration/issues/7)
...
### Alex Olivas (https://github.com/fiedl/icecube-git-migration/issues/8)
...
- [x] to answer questions like “what was the state of combo at 23 October” (because “it has worked then, why doesn’t it now”) is an issue, but a minor one. this is a natural problem with git that we'll just have to get used to.
  ...
  - We should log the commit id and the current date time to a log file when executing the `env-shell.sh` and icetray. This way, we could ask people that need support to grep that logfile (`grep 2019-10-23 /var/log/icetray.log`) and give us the commit id from the output.
...
- [x] really we'll want to provide people with the option to compose meta-projects based on combo. This can be done with svn externals. The only downside is that it's not a clone, so people can't push back and not great for devlopment. But we can live with that.
...
- [x] Add CI pipeline
  ...
  - But considering our bild time, github actions will cost money if the repo is private
...
- A single repo is easier to grasp for newcomers. One single repo, one central README as entrypoint. Get all necessary code with one single `git clone`.
...
- [ ] Update the meta-project svn externals to point to the combo repo's svn bridge
- [ ] Enable github issues
- [ ] Enable private forking
- [ ] Publish announcement of the upcoming change, including howto instructions
- [ ] Make svn read-only and refer to instruction how to proceed
